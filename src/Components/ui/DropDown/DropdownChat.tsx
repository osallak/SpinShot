    ...
    return (
        <div className=" font-sans box-border  inline-block">
            <div className=" bg-cover flex justify-start relativ">
                <button className="py-2 px-4  bg-very-dark-purple  cursor-pointer rounded-md" onClick={down}>
                    <Image src={chatMenu} alt="" />
                </button>
                {isOpen && (
                    <div className="p-4 absolute my-11 mx-8 w-65 bg-very-dark-purple  rounded-r-[20px] rounded-b-[20px]"> {ArrayChat.map((option, index) => (
                            <div
                                key={index}
                            ...
                            >
                                <button className=" flex justify-row  text-pearl opacity-40 hover:opacity-100">
                                    ...
                                    <div className="absolute right-[12px]" onClick={() => setIsOpenChild(!isOpenChild)}>
                                            {option.name === "Mute" ? <Image src={flechIcon} alt="flech"/> : ''}
                                    </div>
                                </button>
                            </div>
                        ))}
