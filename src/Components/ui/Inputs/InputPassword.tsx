...
const InputBorder: React.FC<InputsProps> = ({
  ...
  const handleInputChange = (e: ChangeEvent<HTMLInputElement>) => {
    ...
    const Reg = e.target.value.match(Regexp);
    if (!Reg)
      setisReg(false)
    else
      setisReg(true)
  ...
  return (
    <div
      className={`rounded-2xl w-[${Width}px] h-10 flex flex-row items-center bg-${Color} border-opacity-40 px-2 ${!isReg ? 'border-' + Border : 'border-red-900'} border-${BorderSize}`}
    >
