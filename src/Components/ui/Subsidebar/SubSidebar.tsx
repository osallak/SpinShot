    ...
    <div className="  ml-[150px]  backdrop:blur bg-white/10 space-y-10 w-[30%] h-[1335px] hidden c-gb:block rounded-[20px]  ">
                 <div className="w-full flex flex-col  h-[100px]">
                     ...
                     <div className="w-full flex justify-center items-center">
                         <div className=" bg-pearl w-[80%] opacity-40 border">
                         </div>
                     </div>
                 </div>
                 <div className={`font-Poppins text-pearl text-[1.32rem] `} >
                    {buttons.map((button) => (
                             <div key={button.id}  className={`flex justify-center items-center rounded-2xl  w-full h-20`}>
                                 <button className={` w-[90%] h-full rounded-2xl flex justify-center items-center ${subbackground === button.id ? ' bg-very-dark-purple' : ''}`} onClick={() => handle(button.id, button.route)}>
                                     <div className="w-[80%] h-full flex justify-start items-center c-ml:text-xl 2xl:text-base xl:text-sm text-sm">
                                             {button.text}
                                             {button.text === 'Security' ? <div className={`flex justify-end items-center rounded-2xl h-20 w-[100%]`}> <div className=""  onClick={() => setClick(!isClick)}> {isClick ? <Image src={securityIcon2} alt="" /> : <Image src={securityIcon} alt="" />  }</div></div>: ''}
                                     </div>
                                 </button>
                             </div>
                    ))}
                    {isClick && (
                         <div className=" ">
                             ...
                             <div className={`c-ml:text-xl 2xl:text-base xl:text-sm text-xs w-[60%] rounded-[12px] left-[33%] relative bottom-2 xl:bottom-3 2xl:bottom-4  ${background ? 'bg-very-dark-purple' : 'bg-white/12'}`} onClick={() => setBackground(true)}>
                             ...
                             <div className={` relative flex rounded-[12px] justify-between c-ml:text-xl 2xl:text-base xl:text-sm text-xs  w-[55%] left-[39%] bottom-[65px] xl:bottom-[65px] 2xl:bottom-[70px] `} >
