...
const Sidebar = () => {
    ...
    return (
        <div className={`  hidden c-gb:block w-[150px] h-[1335px] bg-white   backdrop:blur  bg-white/10 rounded-[20px]`}>
                            ...
                            <div className="  flex justify-between flex-col items-center space-y-10">
                                {Icons.map((option, index) => (
                                    <div key={index} className=" opacity-40 hover:opacity-100 ">
                                        {option.route != ("/search") && option.route != ("/notification") ? <button onClick={() => handle(option.route)}> <Image src={option.icon} alt=""/> </button> :  <button onClick={a}> <Image src={option.icon} alt=""/> </button>}
                                    </div>
                                ))}
                            </div>
                    </div>
